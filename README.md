...
quest set goes into the footage Merkle tree — this line claimed that for months
before the claim was checked and found false, so it is worth saying what makes
it true now. Each cutoff builds a leaf for every quest it issued and every
verdict it took, `ghostr verify` re-derives them, and deleting a sealed day's
quest with raw SQL turns `roots OK` into `roots FAILED` at exactly that day.
SPEC §5.4's *"backdating a good streak requires breaking SHA-256 or Bitcoin"* is
a description now rather than an intention.
