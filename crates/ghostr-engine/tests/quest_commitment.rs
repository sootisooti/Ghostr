...
fn ready(dir: &Path) -> (Engine, FixedClock) {
    Engine::init(dir, &passphrase(), Tz::UTC, None, None, cheap()).expect("init");
...
/// Rewrites sealed footage rows, the way someone holding the vault file would.
///
/// The immutability triggers are the *database's* promise (SPEC I2), and a
/// database's promise is only about a file — dropping a trigger takes one
/// statement to anyone with the file and the passphrase. That is the threat
/// these tests are about, and it is why the day's leaves are in a Merkle root
/// and not only behind a trigger.
///
/// Two of the tests below use it as fixture construction rather than as attack:
/// there is no old binary to seal a day with, so the row is written by hand.
fn rewrite_sealed_footage(dir: &Path, sql: &str, params: impl rusqlite::Params) -> usize {
    let db = rusqlite::Connection::open(dir.join(ghostr_store::DB_FILENAME)).expect("open db");
    db.execute_batch(
        "DROP TRIGGER footage_is_immutable;
         DROP TRIGGER footage_is_permanent;",
    )
    .expect("drop triggers");
    let changed = db.execute(sql, params).expect("rewrite");
    db.execute_batch(
...
/// The migration, end to end, in the state a real vault is actually in when it
/// meets this build: days sealed by the old rules, and quests sitting in the
/// database that no day ever committed to.
///
/// Constructed by sealing days before any persona exists — so no quests are
/// issued and the roots are the old ones byte for byte — then stamping those
/// rows the way the old build wrote them, then issuing quests. Verify must pass
/// over the old days *and* over the new one that commits the backlog, because
/// a chain that mixes versions is what every existing user will have.
#[test]
fn days_sealed_under_the_old_rules_still_verify_beside_new_ones() {
    let home = tempfile::tempdir().unwrap();
    let dir = home.path().join("vault");
    let (engine, clock) = ready(&dir);
    ...
    // What the old build wrote. The column did not exist, so its rows take the
    // migration default; setting it here is the same thing by hand.
    let changed = rewrite_sealed_footage(
        &dir,
        "UPDATE footage SET commitment_version = 'memories_only'",
        [],
    );
    assert_eq!(changed, 4, "every sealed day was restamped");

    let engine = Engine::open(&dir, &passphrase()).expect("reopen");
    let report = ops::verify(&engine).expect("verify");
    assert!(
        report.chain_ok && report.roots_ok,
        "a chain sealed under the old rules stopped verifying: {report:?}"
    );

    // Now the new build issues quests. They belong to the next cutoff, not to
    // any day already sealed — those are immutable and their roots are the old
    // ones (I2).
    let candidate = ops::propose_persona(&engine).expect("propose");
    ops::adopt_persona(&engine, &candidate).expect("adopt");
    let today = start_date() + chrono::Duration::days(4);
    ops::issue_quests(&engine, today).expect("issue");
    ops::memoria(&engine, today).expect("seal");

    let dek = engine.dek().expect("dek");
    let all = engine.store().all_footage(dek).expect("footage");
    assert_eq!(
        all.iter()
            .filter(|f| f.commitment.version == CommitmentVersion::MemoriesOnly)
            .count(),
        4
    );
    assert!(
        all.iter()
            .any(|f| f.commitment.version == CommitmentVersion::WithQuests)
    );

    let report = ops::verify(&engine).expect("verify");
    assert!(
        report.chain_ok && report.roots_ok,
        "a chain that mixes commitment versions failed: {report:?}"
    );
}

/// And the version has to be load-bearing, or the test above passes for the
/// boring reason that both versions agree on everything.
///
/// A day sealed *with* quests, restamped as though it were sealed without,
/// must stop verifying: its root contains leaves the old rules do not rebuild.
#[test]
fn restamping_a_day_as_the_old_rules_breaks_its_root() {
    let home = tempfile::tempdir().unwrap();
    let dir = home.path().join("vault");
    let day_seq;
    {
        ...
        rewrite_sealed_footage(
            &dir,
            "UPDATE footage SET commitment_version = 'memories_only' WHERE seq = ?1",
            [i64::try_from(day_seq).unwrap()],
        );
    }
    ...
    assert!(
        !report.roots_ok,
        "the commitment version changed nothing about how a day is rebuilt: {report:?}"
    );
    assert_eq!(report.first_bad_seq, Some(day_seq));
}

/// A commitment version this build has never heard of is refused, not guessed
/// at. Guessing verifies the day under the wrong leaf set and reports
/// tampering, which sends a user looking for an attacker when what they have is
/// an out-of-date binary.
#[test]
fn a_version_from_a_newer_build_is_refused_rather_than_assumed() {
    let home = tempfile::tempdir().unwrap();
    let dir = home.path().join("vault");
    {
        let (engine, clock) = ready(&dir);
        ...
        rewrite_sealed_footage(
            &dir,
            "UPDATE footage SET commitment_version = 'with_quests_and_whatever_comes_next'",
            [],
        );
    }
    ...
    let engine = Engine::open(&dir, &passphrase()).expect("reopen");
    assert!(
        ops::verify(&engine).is_err(),
        "a footage row from a newer build was read as though this build understood it"
    );
}
