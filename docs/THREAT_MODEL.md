...
| Client peeks at the user's answer before scoring | Answer commitment stored before display (I6). The commitment, holdout, and decoy columns are immutable by trigger while the claim beside them is not, so a question edited between issue and verdict fails to reproduce its own commitment and the verdict is refused. The commitment is also a leaf in the day's Merkle root (§7.2), so removing or altering a sealed day's quest breaks that root and every link after it | Requires trusting the binary — see T8 |
| Backdating a good streak | Chain + OTS anchoring (§7): rewriting day 40 breaks every subsequent link. Quests and verdicts are in the root now, so inventing a day's worth of correct answers after the fact is the same problem as rewriting a memory | An attacker who never anchored can forge freely; verifiers must check anchor coverage, not just chain validity |
